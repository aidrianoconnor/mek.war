...
### Moving and Attacking, Converting Move Points to Attack Points
The two basic actions that each unit can take per turn are to move and attack.  

Movement is governed by the number Movement Points (MP) that a unit has (a direct translation of unit Speed).  As noted elsewhere, map terrain and features will impact how many MP must be spent to move into a given hex, ie, just because you have 6 MP available does not mean you can necessarily move 6 hexes.
...
There is no enforcement of rules around whether units must move first and then attack or vice versa, and there is nothing to stop a player from alternating back and forth between the two (move 1 hex, attack, move another hex, attack again, etc), or jumping around between units (move unit A, switch to unit B and attack, go back to unit A and attack, etc).  If any unit has either MP or AP remaining you can always switch back to it and take further action within your turn.  Obviously, once a unit has 0 MP and 0 AP you can't take any further actions with it, but that is the one governing rule around order of actions for any unit within your team.  Your turn only ends when you clicking the Finish Turn link, and you can do so whether you've spent all your units' MP and AP, none at all, or anything in-between.
